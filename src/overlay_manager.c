...
static void create_tiling_overlay_content(GtkWidget *parent_container, AppData *app) {
    ...
    int max_grid_num = app->config.tile_columns * 2;
...
static void create_tiling_grid_overlay(GtkWidget *parent_box, AppData *app) {
    // Create a visual representation of tiling options (no frame - parent already has styling)
    GtkWidget *main_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10);
    gtk_box_pack_start(GTK_BOX(parent_box), main_box, TRUE, TRUE, 0);
    // No border width - padding is handled by dialog_container CSS
    ...
    GtkWidget *grid_container = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
    gtk_box_pack_start(GTK_BOX(main_box), grid_container, FALSE, FALSE, 0);
    ...
    for (int i = 1; i <= tile_columns; i++) {
        ...
        gtk_box_pack_start(GTK_BOX(top_row), cell, TRUE, TRUE, 0);
    }

    // Bottom row
    GtkWidget *bottom_row = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
    ...
    for (int i = tile_columns + 1; i <= tile_columns * 2; i++) {
        ...
        gtk_box_pack_start(GTK_BOX(bottom_row), cell, TRUE, TRUE, 0);
    }

    // Other options
    GtkWidget *other_label = gtk_label_new("<b>Other Options:</b>");
    ...
    gtk_box_pack_start(GTK_BOX(other_box), gtk_label_new("C - Center"), FALSE, FALSE, 0);
}
