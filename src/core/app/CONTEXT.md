...
### Does Not Own
- The behavior of feature subsystems stored inside `AppData`, such as matching, rules, harpoon, projects, files, calc, sinks, or bluetooth.
...
16. Normal shutdown after `gtk_main()` cleans up hotkeys, daemon monitoring,
    ...
    connection, Files async state via `cleanup_files_mode()` including
    cancelling any in-flight fd scan and freeing the cached corpus, the X
    display connection, and any opened log file.
