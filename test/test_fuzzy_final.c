...
int main() {
    ...
    strcpy(windows[1].title, "Feature Flags Android/Kotlin SDK | Feature Flags Docs - Google");
    strcpy(windows[1].instance, "google-chrome");
    strcpy(windows[1].class_name, "Google-chrome");
    windows[1].desktop = 2;
    
    strcpy(windows[2].title, "Diana Drew Lane (DM) - EXAMPLE-STUDIO - Slack - Google C");
    strcpy(windows[2].instance, "google-chrome");
    strcpy(windows[2].class_name, "Google-chrome");
    windows[2].desktop = 2;
    
    strcpy(windows[3].title, "cofi [~/Projects/sample-app] — /home/user/Projects/cofi/CLAUD");
    ...
    // Check if Diana Drew Lane is first
    if (result_count > 0 && strcmp(windows[results[0].index].title, 
                                   "Diana Drew Lane (DM) - EXAMPLE-STUDIO - Slack - Google C") == 0) {
        printf("✓ SUCCESS: Diana Drew Lane appears FIRST with score %d!\n", results[0].score);
    } else {
        printf("✗ FAIL: Diana Drew Lane is NOT first\n");
    }
