...
if [ -f "test/test_filter.c" ]; then
    gcc -o test/test_filter test/test_filter.c src/ui/window_filter.c src/matching/match.c src/core/log/log.c $(pkg-config --cflags --libs gtk+-3.0 x11) -lm 2>/dev/null || echo "Warning: Failed to compile test_filter"
fi
